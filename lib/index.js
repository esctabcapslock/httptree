...
class HttptreePath {
    ...
    patch(callback) { /*if(this.patchFn !==null) throw("Already set"); */ this.patchFn = callback; }
        ...
        if (pathnameF == '') {
            ...
            if (method == 'GET' || method == 'HEAD') {
                if (method == 'GET' && this.getFn) {
                    this.getFn(HTreq, HTres, option);
                    return true;
                }
                else if (method == 'HEAD' && this.headFn) {
                    this.headFn(HTreq, HTres, option);
                    return true;
                }
                else
                    return false;
            ...
            const wrapfn = (fn) => __awaiter(this, void 0, void 0, function* () {
                try {
                    HTreq.rawBody = yield post(req);
                    // const json = JSON.parse((data).toString())
                    fn(HTreq, HTres, option);
                }
                catch (e) {
                    (0, message_1.httpError)(400, res, 'request body is wrong' + e + req.url, 'request body is wrong' + req.url);
                }
            });
    ...
    printpathStructure(dt, end = true) {
        ...
        if (this.deleteFn)
            console.log(dt + hd + bt, 'DELETC', this.deleteFn);
